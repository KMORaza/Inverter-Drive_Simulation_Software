...
public class WaveformVisualizer {
    ...
    private final double[] voltageData = new double[1000];
    private final double[] currentData = new double[1000];
    ...
    public synchronized void updateWaveforms(double[] voltages, double[] currents, double speed, double time) {
        voltageData[dataIndex % 1000] = voltages[0];
        currentData[dataIndex % 1000] = currents[0];
        ...
        if (dataIndex == 1 || dataIndex % 1000 == 0) {
        ...
        gc.stroke();
        gc.setStroke(Color.MAGENTA); /// Plot current
        gc.setLineWidth(1.5);
        gc.beginPath();
        for (int i = startIndex; i < maxPoints; i++) {
            double x = i * width / 1000;
            double y = 2 * height / 3 - currentData[i] * 0.5;
            if (i == startIndex) {
                gc.moveTo(x, y);
            } else {
                double prevX = (i - 1) * width / 1000;
        ...
        gc.stroke();
        gc.setStroke(Color.LIME); /// Plot speed
        gc.setLineWidth(1.5);
        gc.beginPath();
        for (int i = startIndex; i < maxPoints; i++) {
            double x = i * width / 1000;
